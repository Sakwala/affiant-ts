...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| [`@affiant/contract`](packages/contract)                     | the wire types and a vendorable JSON Schema, pinned to a protocol tag                                              | `0.1.0-alpha.1` in this repository; published under `alpha` — `npm view @affiant/contract dist-tags`                             |
| [`@affiant/evidence-card`](packages/evidence-card)           | `<affiant-evidence-card>`, a framework-agnostic Web Component that renders an Evidence Card                        | `0.1.0-alpha.1` in this repository; published under `alpha` — `npm view @affiant/evidence-card dist-tags`                        |
| [`@affiant/core`](packages/core)                             | the gate — Affidavit capture, interceptors, projection, policy, the Docket, decisions                              | `0.1.0-alpha.4` in this repository; published under `alpha` — `npm view @affiant/core dist-tags`; complete for Sequences A and C |
...
| [`@affiant/store-postgres`](packages/store-postgres)         | the Docket on Postgres: two append-only tables and a fold across them, over a postgres.js connection the host owns | `0.1.0-alpha.2` in this repository; published under `alpha` — `npm view @affiant/store-postgres dist-tags`                       |
| [`@affiant/adapter-ai-sdk`](packages/adapter-ai-sdk)         | Affiant tool definitions as an AI SDK `ToolSet`, so a write the model proposes is filed rather than executed       | `0.1.0-alpha.0` in this repository; published under `alpha` — `npm view @affiant/adapter-ai-sdk dist-tags`                       |

Later: `@affiant/adapter-langchain`.
...
`@affiant/contract` and `@affiant/evidence-card` are `0.1.0-alpha.1` in this repository, and `@affiant/core` is `0.1.0-alpha.4`. Each release is published under the `alpha` dist-tag with a provenance attestation that binds it to this repository, the workflow that built it and the commit it was built from, through this repository's publish workflow, which moves the `alpha` dist-tag and no other; `npm view <package> dist-tags` shows what the registry currently serves. `@affiant/conformance-driver` is private and did not go. Publishing stays a separate, deliberate step: [`.github/workflows/publish.yml`](.github/workflows/publish.yml) fires on no push and no tag, a maintainer dispatches it by hand, it defaults to a dry run, and a real upload additionally needs the word `publish` typed into a confirmation input. For `@affiant/core` the condition was exact and mechanical: a public parity report naming what each implementation does not yet pass, and a green, merge-blocking TypeScript conformance driver. Both hold at the rulebook's [`v0.1.3`](https://github.com/Sakwala/affiant-protocol/releases/tag/v0.1.3) tag, the one the committed parity manifest names — see [Conformance](#conformance) below — and a `prepack` guard still fails `npm pack` and `npm publish` until `AFFIANT_ALLOW_PUBLISH=1` is set, which only that workflow sets.
...
The protocol ref this repository pins is in [`packages/contract/protocol/PIN`](packages/contract/protocol/PIN), and everything beside it — both wire versions' schemas, the whole conformance suite, and the four machine-readable formats a driver reads — is a byte-for-byte copy at that ref. A test checksums all 181 vendored copies against `packages/contract/protocol/SHA256SUMS`, and separately fetches the ref itself as one archive and compares every copy against it byte for byte — a mismatch, a missing file, or a network that cannot be reached after retrying all fail that test, never skip it. A second test asserts the three committed generated modules are exactly what the generator produces from those copies. See [CONTRIBUTING.md](CONTRIBUTING.md) for how the pin moves.

The pin is the rulebook's [`v0.1.3`](https://github.com/Sakwala/affiant-protocol/releases/tag/v0.1.3) tag — [`packages/contract/protocol/PIN`](packages/contract/protocol/PIN) is the exact, current source of truth if this line has drifted from it again. It may also hold a full commit, which is what it does while a version's text is on the rulebook's default branch and its tag has not been cut: a commit is as immutable as a tag and, unlike a tag, cannot be moved under a running build.

## Conformance

The rules this implementation is held to are numbered in the rulebook's `INVARIANTS.md`, and they are pinned by a **conformance suite**: 61 declarative documents — a wiring, a sequence of acts, and what must then be true — plus 7 canonical byte vectors fixing the exact bytes and digest a host's execution grant binds to. The documents name no class, no file and no language, and they were promoted from this repository's own test set, byte-identical.
...
Then it does the thing the driver exists for: it asserts that the set of documents this implementation does **not** pass equals [`packages/conformance-driver/conformance/parity/typescript-v0.2.json`](packages/conformance-driver/conformance/parity/typescript-v0.2.json) exactly, in **both** directions, over the union of the two fixture sections the run covers. A document that starts failing and is not listed is a regression, or a rule nobody wrote down. A document that starts passing and is still listed is a gap that has been closed and not published — and a check that caught only the first would let a fix rot unrecorded until the manifest was a document nobody trusted.
...
This is also the guard on publishing. `@affiant/core` goes to npm only when both implementations' parity manifests are public — the .NET line's names what it does not yet pass — and this check is green. Both conditions are now met at the rulebook's [`v0.2.0`](https://github.com/Sakwala/affiant-protocol/releases/tag/v0.2.0) tag: the [.NET parity report](https://github.com/Sakwala/affiant-protocol/blob/v0.2.0/conformance/parity/dotnet-v0.1.json) is public, with its oracle run log alongside it under `conformance/results/`, and this repository's [`typescript-v0.2.json`](packages/conformance-driver/conformance/parity/typescript-v0.2.json) parity manifest records `"failing": []` against that same tag, which is what `packages/contract/protocol/PIN` pins — asserted by the `conformance` job required on `main`. `@affiant/core` went to npm on that basis, on 2026-09-06 at `0.1.0-alpha.0`. A `prepack` guard still fails `npm pack` and `npm publish` until `AFFIANT_ALLOW_PUBLISH=1` is set, and the only thing that sets it is the hand-dispatched publish workflow — which re-runs this suite on all three runtimes first, rather than taking the required check on `main` on trust.
...
Opened 2026-09-04, building in public from day 0. Every package is in the repository against the rulebook's v0.1 wire, and `@affiant/core` is complete for both v0.1 sequences — the pipeline, the Docket, the decision path, and the whole promoted conformance suite green on all three runtimes with the parity manifest asserted in a required CI check. All five publishable packages are, in this repository: `@affiant/core` at `0.1.0-alpha.4`, `@affiant/contract` and `@affiant/evidence-card` at `0.1.0-alpha.1`, `@affiant/store-postgres` at `0.1.0-alpha.2`, and `@affiant/adapter-ai-sdk` at `0.1.0-alpha.0`. Each is published under the `alpha` dist-tag with a provenance attestation through this repository's publish workflow, which moves the `alpha` dist-tag and no other; `npm view <package> dist-tags` shows what each is currently serving. Follow progress in [Discussions](https://github.com/Sakwala/affiant-ts/discussions) and the [Affiant roadmap](https://github.com/Sakwala/affiant/blob/main/ROADMAP.md).
