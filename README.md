...
| [`@affiant/store-postgres`](packages/store-postgres)         | the Docket on Postgres: two append-only tables and a fold across them, over a postgres.js connection the host owns | `0.1.0-alpha.0` on npm under the `alpha` dist-tag — `npm i @affiant/store-postgres@alpha`                       |
