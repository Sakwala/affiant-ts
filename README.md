...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| [`@affiant/contract`](packages/contract)                     | the wire types and a vendorable JSON Schema, pinned to a protocol tag                       | `0.1.0-alpha.0` on npm — `npm i @affiant/contract@alpha`                             |
| [`@affiant/evidence-card`](packages/evidence-card)           | `<affiant-evidence-card>`, a framework-agnostic Web Component that renders an Evidence Card | `0.1.0-alpha.0` on npm — `npm i @affiant/evidence-card@alpha`                        |
| [`@affiant/core`](packages/core)                             | the gate — Affidavit capture, interceptors, projection, policy, the Docket, decisions       | `0.1.0-alpha.0` on npm — `npm i @affiant/core@alpha`; complete for Sequences A and C |
...
Opened 2026-09-04, building in public from day 0. Every package is in the repository against the rulebook's v0.1 wire, and `@affiant/core` is complete for both v0.1 sequences — the pipeline, the Docket, the decision path, and the whole promoted conformance suite green on all three runtimes with the parity manifest asserted in a required CI check. The three published packages are on npm at `0.1.0-alpha.0`, under the `alpha` dist-tag. Follow progress in [Discussions](https://github.com/Sakwala/affiant-ts/discussions) and the [Affiant roadmap](https://github.com/Sakwala/affiant/blob/main/ROADMAP.md).
