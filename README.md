...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`@affiant/contract`](packages/contract)                     | the wire types and a vendorable JSON Schema, pinned to a protocol tag                                              | `0.1.0-alpha.1` on npm under the `alpha` dist-tag — `npm i @affiant/contract@alpha`                                                                                                                   |
| [`@affiant/evidence-card`](packages/evidence-card)           | `<affiant-evidence-card>`, a framework-agnostic Web Component that renders an Evidence Card                        | `0.1.0-alpha.1` on npm under the `alpha` dist-tag — `npm i @affiant/evidence-card@alpha`                                                                                                              |
| [`@affiant/core`](packages/core)                             | the gate — Affidavit capture, interceptors, projection, policy, the Docket, decisions                              | `0.1.0-alpha.1` on npm under the `alpha` dist-tag — `npm i @affiant/core@alpha`; complete for Sequences A and C; `0.1.0-alpha.2` in the repository, unpublished, first release with the next dispatch |
...
| [`@affiant/store-postgres`](packages/store-postgres)         | the Docket on Postgres: two append-only tables and a fold across them, over a postgres.js connection the host owns | `0.1.0-alpha.0` in the repository; unpublished; first release with the next dispatch                                                                                                                  |
| [`@affiant/adapter-ai-sdk`](packages/adapter-ai-sdk)         | Affiant tool definitions as an AI SDK `ToolSet`, so a write the model proposes is filed rather than executed       | `0.1.0-alpha.0` in the repository; unpublished; first release with the next dispatch                                                                                                                  |
...
This is also the guard on publishing. `@affiant/core` goes to npm only when both implementations' parity manifests are public — the .NET line's names what it does not yet pass — and this check is green. Both conditions are now met at the rulebook's [`v0.1.3`](https://github.com/Sakwala/affiant-protocol/releases/tag/v0.1.3) tag: the [.NET parity report](https://github.com/Sakwala/affiant-protocol/blob/v0.1.3/conformance/parity/dotnet-v0.1.json) is public, with its oracle run log alongside it under `conformance/results/`, and this repository's [`typescript-v0.2.json`](packages/conformance-driver/conformance/parity/typescript-v0.2.json) parity manifest records `"failing": []`, today against the rulebook commit its pin names — ahead of that rulebook's `v0.2.0` tag, which is where a version's text lives until the tag is cut — asserted by the `conformance` job required on `main`. `@affiant/core` went to npm on that basis, on 2026-09-06 at `0.1.0-alpha.0`. A `prepack` guard still fails `npm pack` and `npm publish` until `AFFIANT_ALLOW_PUBLISH=1` is set, and the only thing that sets it is the hand-dispatched publish workflow — which re-runs this suite on all three runtimes first, rather than taking the required check on `main` on trust.
...
Opened 2026-09-04, building in public from day 0. Every package is in the repository against the rulebook's v0.1 wire, and `@affiant/core` is complete for both v0.1 sequences — the pipeline, the Docket, the decision path, and the whole promoted conformance suite green on all three runtimes with the parity manifest asserted in a required CI check. The three published packages are on npm at `0.1.0-alpha.1`, under the `alpha` dist-tag. Follow progress in [Discussions](https://github.com/Sakwala/affiant-ts/discussions) and the [Affiant roadmap](https://github.com/Sakwala/affiant/blob/main/ROADMAP.md).
