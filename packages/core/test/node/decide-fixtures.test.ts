...
/**
 * The decision fixtures on disk, and the module the portable suites read them through.
 *
 * The `.json` files are the record a conformance driver reads; the generated module is
 * how `test/decide-fixtures.test.ts` reads the same data on Bun and inside workerd, where
...
const generated = join(packageRoot, "test", "fixtures", "decide.generated.ts");
const generator = join(packageRoot, "scripts", "generate-decide-fixtures.mjs");
...
describe("the decision fixtures on disk", () => {
  it("has one JSON file per fixture in the generated module", () => {
    expect(fixtureFiles().length).toBe(decideFixtures.length);
  });
  ...
  it("mirrors every JSON file into the generated module, value for value", () => {
    ...
    expect(onDisk).toStrictEqual(decideFixtures.map((fixture) => ({ ...fixture })));
  });
  ...
  it("names every fixture for a rule and says what it is for", () => {
    for (const file of fixtureFiles()) {
      ...
      expect(fixture.id, file).toMatch(/^decide\//);
