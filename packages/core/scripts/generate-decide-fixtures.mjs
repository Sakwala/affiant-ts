#!/usr/bin/env node
/**
 * Mirrors the decision fixtures into a TypeScript module the portable suites can import.
 *
 *   test/fixtures/decide/*.json      the fixtures — hand-authored, and the record
 *   test/fixtures/decide.generated.ts  the same fixtures as a module
 *
 * **Why a generated module rather than importing the JSON.** The decision suite runs on
 ...
 * `test/node/decide-fixtures.test.ts` asserts the module and the JSON files still agree,
 * so the mirror cannot drift.
 *
 * Unlike the canonical vectors, nothing here is computed: a decision fixture's expectation
...
const fixtureDir = join(packageRoot, "test", "fixtures", "decide");
...
writeFileSync(join(packageRoot, "test", "fixtures", "decide.generated.ts"), module);

console.log(
  `decide fixtures: mirrored ${String(fixtures.length)} file(s) into test/fixtures/decide.generated.ts`,
);
