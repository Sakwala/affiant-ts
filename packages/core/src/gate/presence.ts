 ...
 * `false`; `null`, objects and arrays have none and never hit. A *hit* is an
 * occurrence of the value text in the utterance under the case-insensitive ordinal
 * comparison below, whose neighbouring code points are absent or are none of a letter
 * (`L*`), a mark (`M*`), a decimal digit (`Nd`) or connector punctuation (`Pc`). The
 * first hit wins, unless the port supplied a span that is itself a hit — the utterance
 * at that span says what the port said it says *and* its neighbours pass the same test
 * — in which case that span is the hit. An empty or whitespace-only value text never
 * hits; an empty utterance is an utterance, and nothing hits in it.
 *
 * **The case table.** Two code points match when they are equal or when their *simple*
 * uppercase mappings are equal — the single-code-point mapping, so a fold never
 * changes a string's length, no culture is consulted, nothing is normalised and no
 * code point is ignorable. In JavaScript that is `toUpperCase()` applied per code
 * point, **keeping the original wherever the result is not the same length**: `"ß"`
 * uppercases to `"SS"`, a full mapping the rule does not apply, so `ß` stays `ß` and
 * does not match `SS`. This is what .NET's `StringComparison.OrdinalIgnoreCase` does,
 * which is why the sibling implementation needs no table of its own.
 *
 ...
 * Multilingual Plane is no more a hit than one abutting an ASCII letter. A combining
 * mark blocks a hit for the same reason — the grapheme the utterance draws there is
 * not the word the value spells (`Cafe` inside a decomposed `Café`) — and `_` blocks
 * one because it joins an identifier (`WZ_BRN`).
 *
 ...
 * rendering of it, a boolean is `true` or `false`.
 *
 * A value with no value text — `null`, an object, an array, and a number with no
 * decimal form — is looked for as the empty string, which never hits, so such a field
 * is `Inferred`.
 */
export function utteranceTextOf(value: JsonValue): string {
  if (typeof value === "string") return value;
  if (typeof value === "boolean") return value ? "true" : "false";
  if (typeof value === "number") return Number.isFinite(value) ? canonicalNumber(value) : "";
  return "";
}

/**
 * `text` with every code point replaced by its simple uppercase mapping.
 *
 * A mapping that is not the same length is not applied, which is what makes the fold
 * simple: the folded string has the length the original had, code unit for code unit,
 * so an offset into the fold is an offset into the utterance.
 */
...
/** Whether the occurrence at `[at, end)` has a boundary on each side. */
function isWholeToken(utterance: string, at: number, end: number): boolean {
  return isBoundaryBefore(utterance, at) && (end >= utterance.length || !joinsAt(utterance, end));
}
...
export function locateInUtterance(
  ...
  if (hint !== null && Number.isInteger(hint.start) && Number.isInteger(hint.end)) {
    ...
    if (
      ...
      isWholeToken(utterance, hint.start, hint.end)
    ) {
  ...
  for (let from = 0; from <= foldedUtterance.length - foldedValue.length;) {
    ...
    if (at < 0) return null;
    if (isWholeToken(utterance, at, at + foldedValue.length)) {
